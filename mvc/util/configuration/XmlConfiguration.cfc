...
component extends="mvc.util.configuration.Configuration" output="false"{
	...
	private void function loadInclude( required Xml xml ){
		var includes = xmlSearch( xml, "/mvc/include/" );
	...
	private void function loadEvent( required Xml xml ){
		var events = xmlSearch( xml , "/mvc/events/event/" );
		var event = {};
		...
		while( iterator.hasNext() ){
			...
			event = newEvent( argumentCollection=next.xmlAttributes );

			triggers = xmlSearch( next, "triggers/trigger/" ).iterator();
			...
			views = xmlSearch( next, "views/view/" ).iterator();
			...
			while( results.hasNext() ){
				...
				event.results.add( result( argumentCollection=results.next().xmlAttributes ) );
	...
	private void function loadInterceptor( required Xml xml ){
		var events = xmlSearch( xml , "/mvc/interceptors/interceptor/" );
		var event = {};
		...
		while( iterator.hasNext() ){
            ...
            while( positions.hasNext() ){
                ...
                triggers = xmlSearch( position, "triggers/trigger/" ).iterator();
                ...
                views = xmlSearch( position, "views/view/" ).iterator();
	...
	private void function loadController( required Xml xml ){
		var controllers = xmlSearch( xml, "mvc/controllers/controller/" );
		var controller = {};
		var iteratorCtrl = "";
		var nextCtrl = "";
		var iterator = "";
	...
	private void function loadConfig( required Xml xml ){
		var configs = xmlSearch( xml , "/mvc/configs/config/" );
		var iterator = "";
		var next = "";
